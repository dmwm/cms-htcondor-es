...
for row in curs.execute("select core_hours, events, read_bytes, dataset, crab_task, month from dataset_popularity"):
...
for dataset, info in datasets.items():
    if 'monthly' not in info: continue
    unused_count = 0
    first_use = False
    for month in months:
        if not first_use:
...
print "Eviction policy of %d months would have resulted in %.2f TB (%d datasets) evicted from disk and %.2fTB (%d datasets) fetched from tape." % (eviction_months, eviction_bytes/1e12, evicted_datasets, fetch_bytes/1e12, fetched_datasets)
